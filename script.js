...
document.getElementById("genButton").addEventListener("click", async (e) => {
  ...
  for (let i = 0; i < excelobjs.length; i++) {
    ...
    for (const section of validSections) {
      ...
      // FIX APPLIED HERE: Look for any matching column header dynamically
      const matchedKey = Object.keys(row).find(key => key.toLowerCase() === fieldLabel);

      let textToDraw = "";
      
      if (fieldLabel === "name") {
        textToDraw = capitalizeName(row["Name"] || row["name"] || "");
      } else if (matchedKey) {
        textToDraw = row[matchedKey];
      } else {
        textToDraw = section.fontSettings.field;
      }
